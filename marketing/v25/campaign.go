...
func (cs *CampaignService) Update(ctx context.Context, c Campaign, fields ...string) (Campaign, error) {
	if c.ID == "" {
		return Campaign{}, errors.New("cannot update a campaign without id")
	}
	...
	url := fb.NewRoute(Version, "/%s", c.ID).Fields(fields...).String()
	c.ID = ""
	err := cs.c.PostJSON(ctx, url, c, &res)
	if err != nil {
		return Campaign{}, err
		...
		return Campaign{}, fmt.Errorf("updating the campaign failed")
	}
