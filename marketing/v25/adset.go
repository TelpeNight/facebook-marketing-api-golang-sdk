...
func (as *AdsetService) Update(ctx context.Context, a Adset, fields ...string) (Adset, error) {
	if a.ID == "" {
		return Adset{}, errors.New("cannot update adset without id")
	}

	if len(fields) == 0 {
		fields = AdsetFields
	} else if !slices.Contains(fields, "id") {
	...
	url := fb.NewRoute(Version, "/%s", a.ID).Fields(fields...).String()
	a.ID = ""
	err := as.c.PostJSON(ctx, url, a, &res)
	if err != nil {
		return Adset{}, err
	} else if err = res.GetError(); err != nil {
		return Adset{}, err
	} else if res.ID == "" {
		return Adset{}, fmt.Errorf("updating the adset failed")
	}

	return res.Adset, nil
}
